...
export default function EditOutletPage() {
  ...
  const fetchOutletData = async () => {
    try {
      ...
      alert(error.response?.data?.detail || 'Failed to load outlet data');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/outlets/${outletId}`, {
        name: formData.name,
      ...
      setSuccessMessage(`Outlet "${formData.name}" has been updated successfully!`);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background p-6 flex items-center justify-center">
        <div className="text-text-secondary">Loading outlet data...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background p-6">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold text-text-primary mb-2">Edit Outlet</h1>
        <p className="text-text-secondary text-sm">Update outlet information</p>
      </div>
